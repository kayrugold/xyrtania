...
// Add showDiagnostics state
code = code.replace(/const \[showHud, setShowHud\] = useState\(true\);/, "const [showHud, setShowHud] = useState(true);\n  const [showDiagnostics, setShowDiagnostics] = useState(false);");

// Add keydown listener
const useEff = `
  useEffect(() => {
    const handleDiagKey = (e) => {
      if (e.key === '\`' || e.key === '~') {
...
`;
code = code.replace(/return \(\s*<div className="relative w-full h-screen/m, useEff + '\n  return (\n    <div className="relative w-full h-screen');

// Add diagnostic panel UI
const diagUI = `
      {showDiagnostics && (
...
`;
code = code.replace(/\{showHud && \(\s*<div className="absolute/m, diagUI + '\n      {showHud && (\n        <div className="absolute');

// Update diagnostics info in the render loop
const renderHook = `
      // Update diagnostics
      if (typeof document !== 'undefined') {
        ...
        if (diagDrawCalls && diagTriangles) {
          diagDrawCalls.innerText = renderer.info.render.calls.toString();
          diagTriangles.innerText = renderer.info.render.triangles.toString();
        }
      }
`;
code = code.replace(/renderer\.render\(hudScene, hudCamera\);/g, "renderer.render(hudScene, hudCamera);\n" + renderHook);
